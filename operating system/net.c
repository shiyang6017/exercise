...
struct sockaddr_in* tcp_sockaddr_in(uint16_t port, const char *str_ip_addr) {
    ...
    bzero((void*)sockaddr_in, sizeof(struct sockaddr_in));
    if (str_ip_addr != NULL) {
...
struct sockaddr_in *tcp_bind(int fd, uint16_t port, const char *str_ip_addr) {

    struct sockaddr_in *sockaddr_in = tcp_sockaddr_in(port, str_ip_addr);

    int err = bind(fd, (struct sockaddr *) sockaddr_in, sizeof(struct sockaddr_in));
    if (err != 0) {
